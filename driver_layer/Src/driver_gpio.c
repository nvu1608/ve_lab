 ...
 * @details Implements GPIO control and EXTI interrupt APIs.
 */
...
static uint8_t prv_gpio_get_port_source(GPIO_TypeDef *instance);
static uint8_t prv_gpio_get_pin_source(uint16_t pin);
static uint32_t prv_gpio_get_exti_line(uint16_t pin);
static IRQn_Type prv_gpio_get_exti_irq(uint16_t pin);

static void prv_gpio_exti_start(gpio_t *dev);
static void prv_gpio_exti_stop(gpio_t *dev);
...
driver_status_t gpio_stop(gpio_t *dev)
{
    ...
    if (dev->exti_state == GPIO_EXTI_ENABLE)
    {
        prv_gpio_exti_stop(dev);
    }

    /*
     * NOTE:
     * GPIO_DeInit() resets the entire GPIO peripheral port.
     * Be careful if multiple pins on the same port are used.
     */
...
bool gpio_read(gpio_t *dev)
{
    ...
    return (GPIO_ReadInputDataBit(dev->instance, dev->pin) != 0u);
}
...
void gpio_exti_irq_process(gpio_t *dev)
{
    ...
    if (EXTI_GetITStatus(prv_gpio_get_exti_line(dev->pin)) != RESET)
    {
        EXTI_ClearITPendingBit(prv_gpio_get_exti_line(dev->pin));
...
static uint8_t prv_gpio_get_port_source(GPIO_TypeDef *instance)
{
    if (instance == GPIOA) return GPIO_PortSourceGPIOA;
    if (instance == GPIOB) return GPIO_PortSourceGPIOB;
    if (instance == GPIOC) return GPIO_PortSourceGPIOC;
    if (instance == GPIOD) return GPIO_PortSourceGPIOD;
    if (instance == GPIOE) return GPIO_PortSourceGPIOE;
...
static void prv_gpio_exti_start(gpio_t *dev)
{
    ...
    GPIO_EXTILineConfig(prv_gpio_get_port_source(dev->instance),
                        prv_gpio_get_pin_source(dev->pin));

    exti.EXTI_Line = prv_gpio_get_exti_line(dev->pin);
    ...
    nvic.NVIC_IRQChannel = prv_gpio_get_exti_irq(dev->pin);
    nvic.NVIC_IRQChannelPreemptionPriority =
        dev->exti_preempt_priority;
    nvic.NVIC_IRQChannelSubPriority =
        dev->exti_sub_priority;

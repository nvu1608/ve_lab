 ...
 * @details Implements DAC initialization, control, and output APIs.
 */
...
static uint16_t prv_dac_get_max_raw_from_align(dac_align_t align);
static uint16_t prv_dac_limit_raw(dac_t *dev, uint16_t raw);
static uint16_t prv_dac_limit_millivolts(dac_t *dev, uint16_t millivolts);
                         ...
                         uint16_t vref_millivolts)
{
    ...
    dev->max_raw = prv_dac_get_max_raw_from_align(align);
...
driver_status_t dac_start(dac_t *dev)
{
    ...
    dac_init_struct.DAC_Trigger = dev->trigger;
    dac_init_struct.DAC_OutputBuffer = dev->output_buffer;

    DAC_Init(dev->channel, &dac_init_struct);
...
driver_status_t dac_write_millivolts(dac_t *dev, uint16_t millivolts)
{
    ...
    millivolts = prv_dac_limit_millivolts(dev, millivolts);
